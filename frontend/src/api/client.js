...
export const api = {
  ...
  adminLaunchCheck: () => request('/admin/launch-check'),
  ...
  dailyReview: (payload = {}) => request('/reports/daily-review', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  externalPushDeliveries: (params = {}) => request(`/settings/external-push-deliveries?${new URLSearchParams(params)}`),
  retryExternalPushDelivery: (deliveryId) => request(`/settings/external-push-deliveries/${encodeURIComponent(deliveryId)}/retry`, { method: 'POST', body: JSON.stringify({}) }),
