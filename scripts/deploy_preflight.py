...
from scripts.production_evidence import EXPECTED_ALEMBIC_REVISION
...
def _actual_env_checks(root: Path) -> list[dict[str, Any]]:
    ...
    live_broker_policy = env.get("LIVE_BROKER_LAUNCH_POLICY", "disabled").strip().lower()
    return [
        ...
        _check("env-file:live-broker-policy-disabled", live_broker_policy == "disabled", "LIVE_BROKER_LAUNCH_POLICY is disabled"),
        _check(
...
def run_preflight(project_root: Path | None = None) -> dict[str, Any]:
    ...
    checks = [
        *file_checks,
        ...
        _check("compose:backend-readiness-healthcheck", "healthcheck:" in compose and "/api/v1/readiness" in compose and "http://127.0.0.1:8765/api/v1/readiness" in compose, "backend readiness healthcheck is configured"),
        _check("compose:migration-seed-schema-gate", all(item in compose for item in ["\n  migration:", "alembic upgrade head", "\n  seed:", "python scripts/bootstrap_mysql_seed.py", "\n  schema-gate:", "python scripts/validate_mysql_schema.py"]), "migration, seed, and schema gate services are configured"),
        _check("compose:schema-gate-dependency", "schema-gate:" in compose and "condition: service_completed_successfully" in compose, "backend waits for schema gate completion"),
        _check("compose:live-broker-policy-disabled", "LIVE_BROKER_LAUNCH_POLICY: ${LIVE_BROKER_LAUNCH_POLICY:-disabled}" in compose, "compose keeps live broker launch policy disabled by default"),
        ...
        _check("env:live-broker-policy-disabled", "LIVE_BROKER_LAUNCH_POLICY=disabled" in env_template, "production template disables live broker launch by default"),
        ...
        _check("evidence:expected-revision", EXPECTED_ALEMBIC_REVISION == "0022_stock_daily_bar_timestamps" and f'"alembic_revision": "{EXPECTED_ALEMBIC_REVISION}"' in _read_text(root / "docs" / "product" / "PRODUCTION_EVIDENCE.example.json"), "production evidence expects the launch schema revision"),
