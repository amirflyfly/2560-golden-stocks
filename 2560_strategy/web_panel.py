...
def render_dashboard(params=None):
    ...
    latest = q(
        f'''SELECT id, pick_date, code, name, pick_price, source_channel, reason_tag, review_status, content_title,
            ...
            FROM picks {where} ORDER BY pick_date DESC, id DESC LIMIT 120''',
        args,
    )
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>宣传票复盘系统 v3.8</title><style>
body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f6f7fb;color:#1f2937;margin:0;padding:24px}} .wrap{{max-width:1540px;margin:0 auto}} .grid{{display:grid;grid-template-columns:repeat(5,1fr);gap:16px}} .grid2{{display:grid;grid-template-columns:repeat(2,1fr);gap:16px}} .grid3{{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}} .card{{background:#fff;border-radius:16px;padding:18px;box-shadow:0 6px 20px rgba(0,0,0,.06)}} h1,h2{{margin:0 0 12px}} .muted{{color:#6b7280;font-size:14px}} .num{{font-size:32px;font-weight:700}} table{{width:100%;border-collapse:collapse}} th,td{{padding:10px;border-bottom:1px solid #eee;text-align:left;font-size:14px;vertical-align:top;white-space:nowrap}} th{{background:#fafafa}} .section{{margin-top:20px}} input,select,textarea{{width:100%;padding:10px;border:1px solid #d1d5db;border-radius:10px;box-sizing:border-box}} textarea{{min-height:90px}} .bigtextarea{{min-height:180px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}} .formgrid3{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}} button{{background:#111827;color:#fff;border:0;border-radius:10px;padding:10px 16px;cursor:pointer}} .hint{{font-size:13px;color:#6b7280}} a.btn{{display:inline-block;background:#111827;color:#fff;text-decoration:none;padding:10px 16px;border-radius:10px;margin-right:8px}} .bar-row,.line-row{{display:grid;grid-template-columns:140px 1fr 62px;gap:10px;align-items:center;margin:8px 0}} .bar-track,.line-track{{height:12px;background:#eef2ff;border-radius:999px;overflow:hidden}} .bar-fill,.line-fill{{height:100%;border-radius:999px}} .bar-label,.bar-value,.line-date,.line-value{{font-size:13px}} .flash{{background:#ecfdf5;color:#065f46;border:1px solid #a7f3d0;padding:10px 12px;border-radius:12px;margin-top:16px}} .inline-form{{display:inline}} .linkbtn{{background:none;color:#2563eb;padding:0 6px;border:none;border-radius:0}} .linkbtn.danger{{color:#dc2626}} .txtbtn{{color:#2563eb;text-decoration:none;padding-right:6px}} .badge{{display:inline-block;padding:4px 8px;border-radius:999px;font-size:12px}} .badge-active{{background:#dcfce7;color:#166534}} .badge-archived{{background:#f3f4f6;color:#4b5563}} .topline{{display:flex;justify-content:space-between;align-items:flex-end;gap:12px;flex-wrap:wrap}} .bulkbar{{display:flex;gap:8px;flex-wrap:wrap;align-items:center}} .bulkbar select{{width:auto;min-width:140px}} .smallbtn{{padding:8px 12px;border-radius:10px}} .tablewrap{{overflow:auto}} .subtle{{font-size:12px;color:#6b7280}} @media (max-width: 980px){{.grid,.grid2,.grid3,.formgrid3{{grid-template-columns:1fr}} body{{padding:16px}} .wrap{{max-width:100%}}}}
</style><script>function toggleAll(source){{document.querySelectorAll('input[name="ids"]').forEach(cb=>cb.checked=source.checked);}} function ensureSelected(form){{const checked=form.querySelectorAll('input[name="ids"]:checked'); if(!checked.length){{alert('请先勾选至少一条记录'); return false;}} return true;}}</script></head><body><div class="wrap"><div class="topline"><div><h1>宣传票复盘系统 v3.8</h1><div class="muted">更新时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} · 已启用密码保护 · 本地端口 {PORT}</div></div><div class="muted">本次新增：批量编辑字段 / CSV导出 / 趋势图</div></div>{flash_html}
...
<div class="grid3 section"><div class="card"><div class="muted">本周新增</div><div class="num">{int(kpi.get('week_new') or 0)}</div></div><div class="card"><div class="muted">近7天新增</div><div class="num">{int(kpi.get('last7_new') or 0)}</div></div><div class="card"><div class="muted">值得复讲率</div><div class="num">{kpi.get('worthy_rate') or 0}%</div></div></div>
<div class="grid3 section"><div class="card"><div class="muted">近30天新增</div><div class="num">{int(kpi.get('last30_new') or 0)}</div></div><div class="card"><div class="muted">平均咨询数</div><div class="num">{kpi.get('avg_inquiry') or 0}</div></div><div class="card"><div class="muted">CSV导出</div><div style="margin-top:12px"><a class="btn" href="{export_csv_href}">导出 CSV</a></div></div></div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code, body, content_type='text/html; charset=utf-8', extra_headers=None):
        ...
        self.send_response(code); self.send_header('Content-Type', content_type); self.send_header('Content-Length', str(len(body_bytes)))
        if extra_headers:
    ...
    def _selected_ids(self, data): return [int(x) for x in as_list(data, 'ids') if str(x).isdigit()]
    def _batch_update(self, sql, ids): return execute_many(sql, [(i,) for i in ids]) if ids else 0

    def do_GET(self):
        parsed = urlparse(self.path)
        if parsed.path == '/health': self._send(200, 'ok', 'text/plain; charset=utf-8'); return
        if parsed.path == '/logout': self._redirect('/login', f'{COOKIE_NAME}=; Path=/; Max-Age=0'); return
        if parsed.path == '/login': self._send(200, render_login()); return
        if not self.authed(): self._redirect('/login'); return
        if parsed.path == '/': self._send(200, render_dashboard(parse_qs(parsed.query))); return
        if parsed.path == '/edit': self._send(200, render_edit_form(q1('SELECT * FROM picks WHERE id=?', (parse_qs(parsed.query).get('id', [''])[0],)))); return
    ...
    def do_POST(self):
        ...
        if self.path == '/login':
            if data.get('password', '') == PANEL_PASSWORD: self._redirect('/', f'{COOKIE_NAME}={PANEL_PASSWORD}; Path=/; HttpOnly')
            else: self._send(200, render_login('密码不对，再试一次'))
            return
        if not self.authed(): self._redirect('/login'); return
        if self.path == '/add':
        ...
        if self.path == '/bulk-import': ids = bulk_import_from_csv(data.get('csv_text', '')); log_action('bulk_import', ids, f'CSV 导入 {len(ids)} 条'); self._redirect('/?imported=1'); return
        if self.path == '/update':
            rid = data.get('id', '')
...
if __name__ == '__main__':
    ...
    print(f'v3.8 server running on http://0.0.0.0:{PORT}')
