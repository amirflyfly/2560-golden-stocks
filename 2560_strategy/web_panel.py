...
def render_leaderboards_page(range_key='30d'):
    range_sql_map = {
    ...
    range_links = ''.join([
        f"<a class='{'navlink nav-active' if range_key == rk else 'navlink'}' href='/leaderboards?range={rk}'>{label}</a>"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/leaderboards':
            ...
            self._send(200, render_leaderboards_page((params.get('range', ['30d'])[0] or '30d'))); return
