...
class Settings:
    ...
    market_data_provider: str = "mootdx"
    market_data_fallbacks: tuple[str, ...] = ("akshare",)
    ...
    def from_env(cls) -> "Settings":
        ...
        market_data_provider = os.getenv("MARKET_DATA_PROVIDER", "mootdx").lower()
        fallbacks = os.getenv("MARKET_DATA_FALLBACKS", "akshare")
        ...
        if market_data_provider == "mock":
            raise RuntimeError("MARKET_DATA_PROVIDER=mock is no longer supported")
        if "mock" in market_data_fallbacks:
            raise RuntimeError("MARKET_DATA_FALLBACKS must not include mock")
