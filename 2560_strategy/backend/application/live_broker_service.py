...
class DisabledBrokerAdapter(BrokerAdapter):
    ...
    def submit_order(self, order_intent: dict, config: "LiveBrokerConfig") -> dict:
        return {"status": "blocked", "reason": "live_trading_disabled", "adapter": self.name}
...
class LiveBrokerService:
    ...
    def broker_status(self, payload: dict | None = None) -> dict:
        ...
        return {
            ...
            "guard": "live orders require env enablement, env dry-run disablement, valid adapter config, and per-request confirmation",
        }
