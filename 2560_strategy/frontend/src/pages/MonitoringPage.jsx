...
const TASK_STATUSES = ['', 'pending', 'running', 'retrying', 'completed', 'failed', 'cancelled', 'stale'];
...
export function MonitoringPage() {
  ...
  function load(page = taskPage, status = taskStatus) {
    setState((prev) => ({ ...prev, loading: true, error: '' }));
    Promise.all([
      api.monitoringOverview(),
      api.monitoringMetrics(),
      api.monitoringLogs({ limit: 80 }),
      api.tasks({ page, page_size: 10, ...(status ? { status } : {}) }),
    ])
  ...
  function changePage(nextPage) {
    ...
    load(normalized, taskStatus);
  }

  function cancelTask(task) {
    api.cancelTask(task.id).then(() => load(taskPage, taskStatus)).catch((error) => setState((prev) => ({ ...prev, error: error.message })));
  }
  ...
  return (
    <main className="page">
      <PageHeader
        ...
        actions={<button type="button" onClick={() => load(taskPage, taskStatus)} disabled={state.loading}>Refresh</button>}
      />
